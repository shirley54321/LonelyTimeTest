...
{
    public class PB_GameStatesManager : GameStatesManager
    {
        [SerializeField] PB_BonusSpecialRules PB_bonusSpecialRules;

        [SerializeField] PB_ReplaceObject PB__replaceObject;
        protected override IEnumerator StateRoundRunning(StatesController attachedStatesController)
        {
            ...
            else
            {
                ...
                PB_bonusSpecialRules.RecordLockedReel(roundResultData.showedIconsId);
            ...
            if (isSpinSucceed)
            {
                ...
                ///啟用與播放動畫
                PB_bonusSpecialRules.OpenAnimation();
            ...
            {
                //Wushih-公版沒有
                PB_bonusSpecialRules.OpenAndCloseFakeReel();
        ...
        protected override IEnumerator WaitForExitingBonusGameTransition(bool isAutoPlay)
        {
            ...
            // transition fade in
            yield return _guiDisplayManager.GetTransitionRunning(TransitionsManager.TransitionType.FadeOut);
            //PB-公版沒有 結束Bonusgame時關閉所有鎖定滾筒圖案
            PB_bonusSpecialRules.LeaveBonusGameunLockReel();
            //公版沒有切換場景物建
            PB__replaceObject.BonusToBase();
            _backgroundManager.ChangeBackground("BaseGame");
            _slotMainBoard.RestoreBaseGameRoundFromPending(statesParameters.BonusWinPerWonBonusLine-100);
