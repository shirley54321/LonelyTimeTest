...
{
    public class WushihReelController : ReelController
    {
        ...
        public override void SetShowedIconsId(int[] showedIconsId)
        {
            ...
            for (int i = 0; i < iconCarriers.Length; i++)
            {
                if (i < showedIconsId.Length && iconCarriers[i] != null)
                {
                    ...
                    //Wushih-公版沒有
                    if (WushihBonusSpecialRules.isAllFakeReel[this.ReelIndex] == false)
                    {
                        if (iconCarriers[i].CurrentIconId == 1 || iconCarriers[i].CurrentIconId == 0)
                        {
                            iconCarriers[i].SR.sortingOrder = (iconCarriers[i].CurrentIconId == 1) ? 2 : 1;
                        }
                        else 
                        {
        ...
        {
            int childOrder = 0;
            if (WushihBonusSpecialRules.isAllFakeReel[this.ReelIndex])
            {
