...
{
    public class WushihAnimatedIconsController : AnimatedIconsController
    {
        /// <summary>
        ...
        public override void DisplayAnimatedIcons(Vector2Int[] coords)
        {
            ...
            foreach (var coordAnimatedIconPair in animatedIcons)
            {
                ...
                coordAnimatedIconPair.Value.transform.localPosition =
                new Vector3(coordAnimatedIconPair.Value.transform.localPosition.x, coordAnimatedIconPair.Value.transform.localPosition.y, GetAnimationZAxis(coordAnimatedIconPair.Key.y));
            }
            foreach (var coordbonusGameAnimatedIcons in BonusGameAnimatedIcons)
            {
                coordbonusGameAnimatedIcons.Value.SetActive(false);//內為舞獅
            }
        }
        ...
        public override void PrepareAnimatedIcons(int[,] showedIconsId, Vector2Int[] coords)
        {
            ...
            {
                int iconId = AnimationIconID(i, showedIconsId, coords);

                Vector2 position = reelsContainerInfoManager.GetReelIconPosition(coords[i]);

                
                if (WushihBonusSpecialRules.isAllFakeReel[coords[i].x] & iconId == 1)
                {
        ...
        protected override GameObject GenerateAnimatedIcon(Vector2 position, int iconId, float deltaPosZ = 0f) 
        {
            ...
            //使用IconID來判斷,改變動畫播放層級，使wild與bonus不備起他動畫遮擋
            animatedIcon.transform.GetComponent<SpriteRenderer>().sortingOrder = (iconId == 1 || iconId == 0) ? 8 : 7;
        ...
        /// <summary>
        /// 調整動畫動畫Z軸高度，使動畫圖層有規則排序
        /// </summary>
        /// <param name="a"></param>
        /// <returns></returns>
        float GetAnimationZAxis (int a)
        {
            float ZAxis = 0.0f;
            switch (a) 
            {
        ...
        int AnimationIconID(int i, int[,] showedIconsId, Vector2Int[] coords) 
        {
            int iconId;
            if (WushihBonusSpecialRules.isAllFakeReel[coords[i].x] == true & showedIconsId[coords[i].x, coords[i].y] == 1)
            {
                int BonusConut = 0;
